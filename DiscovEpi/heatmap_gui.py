...
def produce_heatmap(prot_res, epis, dir, cutoff, progress):
    ...
    for loc_key in prot:
        ...
        for gene in unp:
            ...
            #        name = gene + "_" + str(div+1)
            #        start = i*1000
            #        end = length-1 if length < (i+1)*1000 else (i+1)*1000-1
            #        piece = seq[start:end]
            #        print(len(piece))
            #        print(piece)
            #        print(seq[start-4:end+4])
            #
            #        unp[name] = piece
            #    del unp[gene]
            ...
            progress.emit((list(unp.keys()).index(gene)+1) / len(unp.keys()) * 100)
        ...
        hm_df = pd.DataFrame(hm_score_data, index=unp.keys())
        ...
        plt.title("Heatmap: " + loc_key + ", absolute", fontsize = 20)
        plt.xlabel("Amino acid sequence [absolute position of amino acid]", fontsize = 20)
        path = dir + filename
        print(path)
        plt.savefig(path)
